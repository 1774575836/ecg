...
func main() {
	...
	for pktIndex, _ := range bltPackets {
		fmt.Println(bltPackets[0], bltPackets[1])
		if ret := C.ecg_decode((*C.uchar)(unsafe.Pointer(&bltPackets[pktIndex][0])),
			C.uint(len(bltPackets[pktIndex]))); ret > 0 {
	...
	for {
		...
		//fmt.Println(data[:readSize])
		if ret := C.ecg_decode((*C.uchar)(unsafe.Pointer(&decodeBuffer[0])), C.uint(readSize)); ret > 0 {
